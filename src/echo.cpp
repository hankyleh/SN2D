...
void io::echo(Parameters input_data) {
  ...
  for (int i = 0; i < input_data.I; i++) {
    for (int j = 0; j < input_data.J; j++) {
      ...
      buffer =
          buffer + io::add_spaces(std::to_string(input_data.mat_id[i][j]), 10);
      ...
      buffer = buffer + io::add_spaces(os.str(), 6);
      buffer =
          buffer + io::add_spaces(std::to_string(input_data.sig_tot[i][j]), 10);
      buffer =
          buffer + io::add_spaces(std::to_string(input_data.sig_sca[i][j]), 10);
